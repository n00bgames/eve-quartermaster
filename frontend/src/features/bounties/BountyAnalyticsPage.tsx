...
export function BountyAnalyticsPage({ api, timeZone, formatDateTime }: { api: ApiClient; timeZone: string; formatDateTime: (value: string) => string }) {
  ...
  const viewingAllEnrolledPilots = data?.scope === "all_enrolled_pilots";

  return <section className="panel stacked bounty-analytics-page">
    <div className="section-heading"><div><h3>Bounty Analytics</h3><p>Persistent NPC bounty ticks, pilot comparisons, and authoritative corporation-tax reconciliation from character wallet journals.</p></div><div className="button-row compact">{data?.engine_used && <span className="version-badge">{data.engine_used === "rust" ? "Rust analytics core" : data.engine_used}</span>}<button type="button" disabled={loading} onClick={() => void exportCsv()}><Download size={16} />Export CSV</button><button type="button" disabled={loading} onClick={() => void load()}><RefreshCw size={16} />Refresh</button></div></div>
    <div className="bounty-privacy-note"><ShieldCheck size={19} /><div><strong>{viewingAllEnrolledPilots ? "Host and administrator pilot scope" : "Private, account-scoped analytics"}</strong><span>{viewingAllEnrolledPilots ? "This view includes every enrolled pilot whose wallet history and synchronization are enabled. Corporation filters use the corporation preserved with each historical journal row." : "This module includes only wallet-visible characters connected to your account. Hosts and administrators can view all enrolled pilots; other accounts remain isolated. Corporation filters use the corporation preserved with each historical journal row."}</span></div></div>
    ...
    <div className="bounty-filter-grid">
      <label>View<select value={corporationId} onChange={(event) => { setCorporationId(event.target.value); setCharacterId(""); }}><option value="">{viewingAllEnrolledPilots ? "All enrolled pilots" : "All my pilots"}</option>{data?.corporations.map((row) => <option key={row.corporation_eve_id} value={row.corporation_eve_id}>{row.corporation_name}</option>)}</select></label>
