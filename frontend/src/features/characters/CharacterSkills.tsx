...
function CharacterSkillProfiles({ currentUser, api, Metric, CharacterHoverName }: CharacterSkillsProps) {
  ...
  function downloadAllSkills(format: "csv" | "json") {
    const generatedAt = new Date();
    const content = format === "csv"
      ? allCharacterSkillsCsv(profiles)
      : allCharacterSkillsJson(profiles, generatedAt);
    const url = URL.createObjectURL(new Blob([content], {
      type: format === "csv" ? "text/csv;charset=utf-8" : "application/json;charset=utf-8",
    }));
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = allCharacterSkillsFilename(format, generatedAt);
    document.body.appendChild(anchor);
    anchor.click();
    anchor.remove();
    URL.revokeObjectURL(url);
    setSkillError(null);
    setMessage(`Exported all visible character skills as ${format.toUpperCase()}.`);
  }
