...
type SyncKind = "assets" | "skills" | "fittings" | "contracts";
...
export function CharactersPage({
  ...
  const syncButton = (token: CharacterDossierToken, kind: SyncKind, label: string, enabled: boolean) => (
    <button type="button" disabled={!token.can_sync || !enabled || busySync !== null} onClick={() => void runCharacterSync(kind, token)}>
  ...
  return (
    <section className="panel stacked">
      <div className="section-heading">
        ...
        <button type="button" onClick={() => void loadCharacters(selectedCharacterId).then((nextId) => loadDossier(nextId))}>Refresh</button>
      </div>
