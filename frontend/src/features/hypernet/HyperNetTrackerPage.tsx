import { BarChart3, Calculator, Clock3, Coins, Dice5, History, LayoutGrid, List, Plus, RefreshCw, TicketCheck, Trophy, XCircle } from "lucide-react";
...
type View = { kind: "board" | "new" | "new-bid" } | { kind: "detail"; id: number };
...
export function HyperNetTrackerPage({ api }: { api: ApiClient }) {
  ...
  useEffect(() => { if (route.kind === "board") void loadBoard(); else if (route.kind === "detail") void loadDetail(route.id); }, [route, status, bidOutcome, loadBoard, loadDetail]);
