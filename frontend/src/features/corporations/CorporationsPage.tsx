...
export function CorporationsPage({ api, loadAssets, EveEntityIcon }: CorporationsPageProps) {
  ...
  async function loadCorporations() {
    setCorporations(await api<EqmCorporation[]>(`/corporations${showHidden ? "?include_hidden=true" : ""}`));
  }
  ...
  async function updateCorporationHidden(corporation: EqmCorporation, hidden: boolean) {
    setBusyCorporationId(corporation.id);
    setCorpError(null);
    setMessage(`${hidden ? "Hiding" : "Showing"} ${corporation.name}...`);

    try {
      await api<EqmCorporation>(`/corporations/${corporation.id}`, { method: "PATCH", body: JSON.stringify({ hide_from_corporation_list: hidden }) });
      setMessage(`${corporation.name} ${hidden ? "hidden from" : "restored to"} the corporation page.`);
      await loadCorporations();
    } catch (err) {
      setCorpError(err instanceof Error ? err.message : "Corporation visibility update failed");
    } finally {
      setBusyCorporationId(null);
    }
  }

  async function updateCorporationAnalytics(corporation: EqmCorporation, excluded: boolean) {
    setBusyCorporationId(corporation.id);
    setCorpError(null);
    setMessage(`${excluded ? "Excluding" : "Including"} ${corporation.name} ${excluded ? "from" : "in"} analytics...`);

    try {
      await api<EqmCorporation>(`/corporations/${corporation.id}`, { method: "PATCH", body: JSON.stringify({ exclude_from_analytics: excluded }) });
      setMessage(`${corporation.name} ${excluded ? "excluded from" : "included in"} analytics.`);
      await loadCorporations();
    } catch (err) {
      setCorpError(err instanceof Error ? err.message : "Corporation analytics setting failed");
    } finally {
      setBusyCorporationId(null);
    }
  }
