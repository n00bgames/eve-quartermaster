...
export function AnalyticsPlatform({ currentUser, api, Metric }: AnalyticsPlatformProps) {
  ...
  async function loadAnalytics(selectedDays = days, selectedScope = analyticsScope) {
    ...
    setLoading(true);
    try {
      const [scopeMode, scopeCorporationId] = selectedScope.startsWith("corporation:") ? ["corporation", selectedScope.split(":", 2)[1]] : [selectedScope, null];
      const scopeQuery = `scope=${encodeURIComponent(scopeMode)}${scopeCorporationId ? `&corporation_id=${encodeURIComponent(scopeCorporationId)}` : ""}`;
