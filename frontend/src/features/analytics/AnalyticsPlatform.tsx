...
export function AnalyticsPlatform({ currentUser, api, Metric }: AnalyticsPlatformProps) {
  ...
  async function loadAnalytics(selectedDays = days) {
    ...
    setLoading(true);
    try {
      const [nextSummary, nextCatalog, nextCorporationScope, nextPlanetary, nextFinancial] = await Promise.all([api<AnalyticsSummary>(`/analytics/summary?days=${selectedDays}`), api<MetricCatalogItem[]>("/analytics/metrics"), api<AnalyticsCorporationScopeResponse>("/analytics/corporations"), api<PlanetaryAnalytics>(`/analytics/planetary-industry?days=${selectedDays}`), api<FinancialAnalytics>(`/analytics/financial?days=${selectedDays}`)]);
