...
import type { AnalyticsCorporationScopeResponse, AnalyticsPoint, AnalyticsSummary, DuplicateBlueprint, MetricCatalogItem } from "../../types/analytics";
...
export function AnalyticsPlatform({ currentUser, api, Metric }: AnalyticsPlatformProps) {
  ...
  async function loadAnalytics(selectedDays = days) {
    ...
    const [nextSummary, nextCatalog, nextCorporationScope] = await Promise.all([api<AnalyticsSummary>(`/analytics/summary?days=${selectedDays}`), api<MetricCatalogItem[]>("/analytics/metrics"), api<AnalyticsCorporationScopeResponse>("/analytics/corporations")]);
