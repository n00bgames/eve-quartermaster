...
type Asset = { id: number; ownership_entity_id: number; owner_name: string; type_id: number; type_name: string; quantity: number; location_name?: string; location_flag?: string; source: string; parent_asset_item_id?: number; parent_asset_type_name?: string };
...
type CorporationToken = { token_id: number; character_name: string; user_display_name: string; has_corporation_asset_scope: boolean; can_sync: boolean };
type EqmCorporation = { id: number; corporation_id: number; name: string; ticker?: string | null; alliance_name?: string | null; ceo_character_eve_id?: number | null; last_synced_at?: string | null; asset_rows: number; last_asset_sync_at?: string | null; last_asset_sync_status?: string | null; last_asset_sync_message?: string | null; eligible_tokens: CorporationToken[] };
...
function Corporations({ loadAssets }: { loadAssets: () => Promise<void> }) {
  ...
  return <section className="panel stacked"><div className="section-heading"><h3>Corporations</h3><button type="button" onClick={() => void refreshCorporationLinks()}>Refresh corporation links</button></div>{message && <div className="notice inline">{message}</div>}{corpError && <div className="mini-alert">{corpError}</div>}<div className="card-list corporation-list">{corporations.map((corporation) => <article key={corporation.id}><strong>{corporation.name}{corporation.ticker ? ` [${corporation.ticker}]` : ""}</strong><span>{corporation.alliance_name ?? "No alliance"} · Corp ID {corporation.corporation_id}</span><span>{corporation.asset_rows.toLocaleString()} tracked asset rows</span><span>Last corp sync {corporation.last_asset_sync_at ? `${new Date(corporation.last_asset_sync_at).toLocaleString()} (${corporation.last_asset_sync_status ?? "sync"})` : "never"}</span>{corporation.last_asset_sync_message && <code>{corporation.last_asset_sync_message}</code>}<div className="choice-list"><span>Corp asset tokens</span>{corporation.eligible_tokens.length > 0 ? corporation.eligible_tokens.map((token) => <div className="token-row" key={token.token_id}><span>{token.character_name} · {token.user_display_name}</span>{token.has_corporation_asset_scope ? <button type="button" disabled={!token.can_sync || busyTokenId === token.token_id} onClick={() => void syncCorporationAssets(token, corporation)}>{busyTokenId === token.token_id ? "Syncing" : "Sync corporation assets"}</button> : <span className="scope-warn">Missing esi-assets.read_corporation_assets.v1</span>}</div>) : <p className="muted">No linked character tokens found for this corporation yet.</p>}</div></article>)}</div>{corporations.length === 0 && <p className="empty">No corporations imported or linked yet. Re-link a CEO/director through EVE SSO to populate this list.</p>}</section>;
}
...
function AssetTable({ assets }: { assets: Asset[] }) {
  ...
  }, [assets, filter, sortKey, sortDirection]);
