...
<p align="center">
  ...
  <img alt="Version" src="https://img.shields.io/badge/version-1.0.1-4fb3c7?style=for-the-badge">
...
The 1.0 source build brings the PI Operation Planner, corporate supply monitoring, and production-chain calculations into the wider operations suite. See the [v1.0 release notes](docs/releases/v1.0.md) for **The Bigger Slice of PI Release!** The current application/package version is `1.0.1`, adding the Navigation distance and rough warp-time calculator. See the [changelog](CHANGELOG.md) for patch details.
...
- **System Distances & Warp Time** at the bottom of Navigation: search for a system, choose a starting object and destination, and compare AU distances across gates, NPC stations, accessible Upwell structures, planets, moons and belts. Enter warp speed and align time for a rough reference of `distance ÷ speed + align time`; acceleration and deceleration are excluded. [Data coverage and setup](docs/system-distances.md).
...
### Navigation Threat Analytics Rust engine

`EQM_THREAT_ANALYTICS_ENGINE` controls the shared industrial-threat and PvP-intel reducer. Python retains zKill/ESI retrieval, caching, SDE/name/location enrichment, retention, and system access. Rust owns exact-cent destroyed-value totals, deterministic risk scoring, and victim hull, UTC period, attacker/victim organization, location, final-blow hull, and attacker-group-size rankings.

- `python` uses the reference Python reducers.
- `shadow` serves Python while requiring exact parity across every total, score, label, and ranking.
- `rust` serves Rust results and automatically falls back to Python on a binary error, invalid contract, or timeout.
...
### Battle Reports Rust engine

`EQM_BATTLE_REPORT_ENGINE` controls deterministic engagement reconstruction. Python retains pilot permissions, retained-killmail queries, reporting-window selection, ESI/SDE/name enrichment, report history, and immutable public-share persistence. Rust owns affiliation-connected killmail selection, side-graph conflict resolution, manual pilot/organization override precedence, teams, participants, timeline, composition, totals, and efficiency calculations.

- `python` uses the reference Battle Report builder.
- `shadow` serves Python while requiring exact full-report parity from Rust.
- `rust` serves Rust results and automatically falls back to the complete Python builder on a binary error, invalid contract, or timeout.
...
docker compose --profile test run --rm eqm-core-tests cargo test --locked --test battle_reports_contract
docker compose --profile test run --rm backend-tests python -m pytest -q tests/test_battle_reports.py tests/test_battle_report_engine.py
```
