...
- Historical analytics foundation with scope-aware, hourly-coalesced snapshot runs, selectable full or change-only retention, metric metadata/versioning, baseline-aware deltas, honest range coverage, exports, composable widgets, and host-controlled legacy-history compaction.
...
Individual wallet balances and history are always visible only to the EQM account that owns the character. Directors, administrators, and hosts do not receive another pilot's raw balance or personal wallet timeline. EQM does not provide a richest-pilot leaderboard.
