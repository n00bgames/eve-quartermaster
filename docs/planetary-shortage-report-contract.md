...
EQM's PI shortage evaluator is a pure, deterministic calculation over a Planetary Industry payload. The Rust-backed API, browser reference calculation, and Rust parity harness emit the same `eqm.planetary-shortage-report.v2` document for the same payload, target, and generated timestamp.
...
- `inventory_days_at_demand = total_inventory / configured_demand_per_day`.
- `runway_days_at_net_shortfall = total_inventory / net_shortfall_per_day`; it is `null` when configured throughput is balanced.
...
- Targeted reports recursively include the selected configured product's recipe inputs and their factory-produced dependencies. Network reports include every commodity with configured demand or supply. Targeted reports also include the selected finished product.
...
## Corporate stock and surplus (v2)

`hangar_inventory` is the authorized selected hangar snapshot; `total_inventory = projected_inventory + hangar_inventory`. Daily throughput is unaffected by inventory. `net_surplus_per_day = max(0, supply - demand)`. Coverage and inventory days are null for outputs without demand. See `pi-supply-and-production.md` for calculator semantics and production verification.
