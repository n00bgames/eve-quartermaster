from __future__ import annotations

from unittest.mock import patch

from app.services.planetary_analytics_engine import evaluate_planetary_analytics_with_engine


def summary_fixture() -> dict:
    return {
        "days": 30,
        "has_history": True,
        "cards": {"estimated_volume": 10.0, "current_volume_per_day": 2.0, "product_count": 1, "character_count": 1},
        "tiers": [],
        "products": [],
        "character_products": [],
    }


@patch("app.services.planetary_analytics_engine._run_rust")
def test_rust_mode_is_authoritative_and_keeps_python_lazy(run_rust) -> None:
    run_rust.return_value = summary_fixture()

    def unused_reference() -> dict:
        raise AssertionError("Rust authority should not execute the Python reference")

    result = evaluate_planetary_analytics_with_engine(
        payload={"schema_version": "eqm.planetary-analytics.v1"},
        python_result=unused_reference,
        engine="rust",
    )

    assert result["engine_used"] == "rust"
    assert result["cards"]["estimated_volume"] == 10.0


@patch("app.services.planetary_analytics_engine._run_rust")
def test_shadow_mode_reports_exact_parity(run_rust) -> None:
    run_rust.return_value = summary_fixture()
    result = evaluate_planetary_analytics_with_engine(
        payload={"schema_version": "eqm.planetary-analytics.v1"},
        python_result=summary_fixture,
        engine="shadow",
    )
    assert result["engine_used"] == "python-shadow"
    assert result["engine_shadow_match"] is True


@patch("app.services.planetary_analytics_engine._run_rust", side_effect=FileNotFoundError("missing"))
def test_rust_failure_lazily_uses_python_fallback(_run_rust) -> None:
    calls = 0

    def reference() -> dict:
        nonlocal calls
        calls += 1
        return summary_fixture()

    result = evaluate_planetary_analytics_with_engine(
        payload={"schema_version": "eqm.planetary-analytics.v1"},
        python_result=reference,
        engine="rust",
    )
    assert calls == 1
    assert result["engine_used"] == "python-fallback"
    assert "missing" in result["engine_fallback_reason"]
