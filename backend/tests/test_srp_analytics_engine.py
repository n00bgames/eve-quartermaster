...
def result_fixture() -> dict:
    return {
        ...
        "breakdowns": {},
        "top": {},
        "quality": {"unvalued_count": 0},
    }


@patch("app.services.srp_analytics_engine.subprocess.run")
def test_rust_mode_sends_versioned_rows_over_stdin(run) -> None:
    payload = {"schema_version": "eqm.srp-analytics-input.v1", "rows": []}
    run.return_value = SimpleNamespace(returncode=0, stderr="", stdout=json.dumps(result_fixture()))
    result = evaluate_srp_analytics_with_engine(
        payload=payload,
    ...
    assert run.call_args.args[0] == ["/eqm-core", "srp-analytics", "--input", "-"]
    assert json.loads(run.call_args.kwargs["input"]) == payload


@patch("app.services.srp_analytics_engine._run_rust")
def test_shadow_mode_requires_exact_parity(run_rust) -> None:
    run_rust.return_value = result_fixture()
    result = evaluate_srp_analytics_with_engine(payload={}, python_result=result_fixture(), engine="shadow")
    assert result["engine_used"] == "python-shadow"
    assert result["engine_shadow_match"] is True


@patch("app.services.srp_analytics_engine._run_rust", side_effect=FileNotFoundError("missing"))
def test_rust_failure_lazily_uses_python_fallback(_run_rust) -> None:
    result = evaluate_srp_analytics_with_engine(payload={}, python_result=result_fixture, engine="rust")
