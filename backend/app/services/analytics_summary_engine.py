...
OUTPUT_SCHEMA = "eqm.analytics-summary-output.v1"
REQUIRED_FIELDS = {
    "cards",
    "change_composition",
    "top_sp_gainers",
...
def _run_rust(binary: str, timeout: float, payload: dict[str, Any]) -> dict[str, Any]:
    completed = subprocess.run(
        [binary, "analytics-summary", "--input", "-"],
    ...
    if result.get("schema_version") != OUTPUT_SCHEMA:
        raise RuntimeError("eqm-core returned an unsupported analytics summary schema")
    if not REQUIRED_FIELDS.issubset(result):
        raise RuntimeError("eqm-core returned an incomplete analytics summary")
    return result
...
def _difference_paths(left: Any, right: Any, path: str = "analytics", limit: int = 16) -> list[str]:
    if limit <= 0:
        return []
...
def evaluate_analytics_summary_with_engine(
    *,
    ...
    requested = (engine or settings.eqm_analytics_engine).strip().lower()
    if requested not in VALID_ENGINES:
        logger.warning("Unknown EQM analytics engine %r; using Python", requested)
        requested = "python"
    ...
    if requested == "shadow":
        ...
        try:
            ...
            if not match:
                logger.warning(
                    "Analytics summary shadow mismatch python=%s rust=%s fields=%s",
                    _digest(_canonical(result)),
                    _digest(_canonical(rust_result)),
                    ",".join(differences),
                )
            result.update(engine_requested="shadow", engine_used="python-shadow", engine_shadow_match=match)
        except Exception as error:
            logger.exception("Rust analytics summary shadow failed; Python result retained")
            result.update(
    ...
    try:
        ...
        logger.exception("Rust analytics summary failed; falling back to Python")
        result = copy.deepcopy(reference_result())
        result.update(
