...
def _run_rust(binary: str, timeout: float, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if result.get("schema_version") != OUTPUT_SCHEMA:
        raise RuntimeError("eqm-core returned an unsupported settlement math schema")
    if not {"participants", "outputs", "participant_payout_total_cents", "unallocated_cents"}.issubset(result):
        raise RuntimeError("eqm-core returned an incomplete settlement math result")
    return result
...
def evaluate_settlement_math_with_engine(
    *,
    ...
    requested = (engine or settings.eqm_settlement_math_engine).strip().lower()
    if requested not in VALID_ENGINES:
        logger.warning("Unknown EQM settlement math engine %r; using Python", requested)
        requested = "python"
    ...
    if requested == "shadow":
        ...
        try:
            ...
            if not match:
                logger.warning("Settlement math shadow mismatch")
            result.update(engine_requested="shadow", engine_used="python-shadow", engine_shadow_match=match)
        except Exception as error:
            logger.exception("Rust settlement math shadow failed; Python result retained")
    ...
    try:
        ...
        logger.exception("Rust settlement math failed; falling back to Python")
        result = copy.deepcopy(reference())
