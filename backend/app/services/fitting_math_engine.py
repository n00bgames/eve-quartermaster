...
def _run_rust(binary: str, timeout: float, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if result.get("schema_version") != "eqm.fitting-math-output.v1":
        raise RuntimeError("eqm-core returned an unsupported fitting math schema")
    if not {"stacking_cases", "capacitor_cases"}.issubset(result):
        raise RuntimeError("eqm-core returned an incomplete fitting math result")
    return result
...
def evaluate_fitting_math_with_engine(
    *,
    ...
    if requested == "shadow":
        ...
        try:
            ...
            if not match:
                logger.warning(
                    "Fitting math shadow mismatch python=%s rust=%s",
            ...
            logger.exception("Rust fitting math shadow failed; Python result retained")
            result.update(
                engine_requested="shadow",
    ...
    try:
        ...
        logger.exception("Rust fitting math failed; falling back to Python")
        result = copy.deepcopy(python_result)
        result.update(
