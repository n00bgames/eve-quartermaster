...
def _run_rust(payload: dict[str, Any], *, binary: str, timeout: float) -> dict[str, Any]:
    completed = subprocess.run(
        [binary, "planetary-analytics", "--input", "-"],
    ...
    if completed.returncode != 0:
        raise RuntimeError((completed.stderr or completed.stdout or "Rust planetary analytics failed").strip())
    result = json.loads(completed.stdout)
    if result.get("schema_version") != "eqm.planetary-analytics.v1":
        raise RuntimeError("Rust planetary analytics returned an unsupported schema")
    result.pop("schema_version", None)
    return result


def evaluate_planetary_analytics_with_engine(
    *,
    ...
    requested = (engine or settings.eqm_planetary_analytics_engine).strip().lower()
    if requested not in VALID_ENGINES:
        logger.warning("Unknown EQM planetary analytics engine %r; using Python", requested)
        requested = "python"
    ...
    try:
        ...
        logger.exception("Rust planetary analytics failed; Python result retained")
        result = {
            **reference(),
    ...
    if requested == "shadow":
        ...
        if not match:
            logger.warning("Planetary analytics shadow mismatch")
        return {
            **python_value,
